...
export const createReview = (resData) => {
  ...
  return async (dispatch) => {
    try {
      const response = await api.post('/public/reviews/create', 
        resData);
...
export const getAllReviews = (productId) => {
  return async (dispatch) => {
    try {
      const response = await api.get(`/public/reviews/product/${productId}`);
...
export const createRating = (resData) => {
  return async (dispatch) => {
    try {
      const response = await api.post('/public/ratings/create', 
        resData);
...
export const getAllRatings = (productId) => {
  ...
  return async (dispatch) => {
    try {
      const response = await api.get(`/public/ratings/product/${productId}`, {
